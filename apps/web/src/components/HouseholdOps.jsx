...
export function HouseholdOps({ expenses = [], chores = [], mode, onAddExpense, onToggleChore }) {
  ...
  // Kanban task creation form state
    ...
    <section className="tab-panel active">
      ...
      <div className="hero-3d-banner">
        <div className="hero-3d-text">
          <h3>Operaciones del Hogar & Kanban de Tareas 📋</h3>
          <p>Gestiona finanzas compartidas y asigna tareas del hogar con Kanban interactivo en <span className="city-highlight">{currentCity} ({activeCurrencyCode})</span>.</p>
        </div>
        <img 
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem', background: '#f1f5f9', padding: '6px', borderRadius: '9999px', width: 'fit-content' }}>
        ...
        >
          <i className="fa-solid fa-square-kanban"></i> Tablero Kanban de Tareas ({safeChores.length})
        </button>
        <button 
          ...
          style={{ borderRadius: '9999px', border: 'none' }}
        >
          <i className="fa-solid fa-wallet"></i> Registro de Gastos del Hogar ({safeExpenses.length})
        </button>
      </div>
      ...
      {activeSubTab === 'kanban' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {/* Create Task & AI Randomizer Action Bar */}
          <div className="expense-card" style={{ padding: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.75rem' }}>
              <h3 style={{ fontSize: '1.15rem', fontWeight: 800, margin: 0 }}>
                <i className="fa-solid fa-plus-circle text-coral"></i> Agregar Nueva Tarea al Tablero
              </h3>
              <button className="btn btn-secondary btn-sm" onClick={randomizeTasks} title="Sortear responsables equitativamente con IA">
                <i className="fa-solid fa-dice text-indigo"></i> 🎲 Sortear Responsables con IA
              </button>
            </div>

            <form onSubmit={handleAddTask} className="form-row" style={{ alignItems: 'flex-end' }}>
              <div className="form-group" style={{ margin: 0, flex: 2 }}>
                <label>Nombre de la Tarea del Hogar</label>
                <input 
                  type="text" 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            ...
            <div className="vault-card" style={{ background: '#f8fafc', border: '1px solid #e2e8f0', minHeight: '350px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '2px solid #cbd5e1', paddingBottom: '0.5rem' }}>
                ...
                <span className="event-badge" style={{ background: '#e2e8f0', color: 'var(--text-muted)' }}>To Do</span>
              </div>
            ...
            <div className="vault-card" style={{ background: '#fffcf5', border: '1px solid #fde68a', minHeight: '350px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '2px solid #f59e0b', paddingBottom: '0.5rem' }}>
                ...
                <span className="event-badge" style={{ background: 'rgba(245, 158, 11, 0.15)', color: 'var(--accent-amber)' }}>In Progress</span>
              </div>
            ...
            <div className="vault-card" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', minHeight: '350px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '2px solid #10b981', paddingBottom: '0.5rem' }}>
                ...
                <span className="event-badge" style={{ background: 'rgba(16, 185, 129, 0.15)', color: 'var(--accent-emerald)' }}>Done</span>
              </div>
      ...
      {/* VIEW 2: EXPENSES REGISTRY & 50/50 SPLIT */}
